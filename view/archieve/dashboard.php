...
<div class="d-flex align-items-center justify-content-between mb-4">
  <h1 class="h3 mb-0">
    <i class="fa-solid fa-bolt"></i> ESD-EOS Management
  </h1>
</div>
      ...
      <table class="table table-striped mb-0">
        ...
        <tbody>
          <?php if (empty($ESDchecker)): ?>
            <tr>
              <td colspan="8" class="text-center text-muted">No records yet.</td>
            </tr>
          <?php else: ?>
            <?php $no = 1; ?>
            <?php foreach ($ESDchecker as $row): ?>
              <tr>
                <td><?= $no++ ?></td>
                <td><?= htmlspecialchars($row['machine_location'] ?? '') ?></td>
                <td><?= htmlspecialchars($row['system_type'] ?? '') ?></td>
              </tr>
            <?php endforeach; ?>
          <?php endif; ?>
        </tbody>
          ...
          <?php else: ?>
            ...
            <?php foreach ($recentChecks as $row): ?>
              <tr>
                ...
                <td><?= htmlspecialchars($row['start_at'] ?? '') ?></td>
